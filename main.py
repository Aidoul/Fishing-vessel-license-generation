...
from plate_number import random_select

from plate_number import generate_plate_number_blue
from plate_number import letters, digits


def get_location_data(length, split_id, height=140):
    """
    获取车牌号码在底牌中的位置
    length: 车牌字符数，
    split_id: 分割空隙
...
class MultiPlateGenerator:
    ...
    def get_location_multi(self, plate_number, height=140):
        ...
        split_id = 2
        print(self.location_xys['{}_{}_{}'.format(length, split_id, height)])
    ...
    def generate_plate_number(self):
            ...
            # 蓝牌
        plate_number = generate_plate_number_blue(length=9)

        # 车牌底板颜色
        bg_color = random_select(['blue'] )


        return plate_number, bg_color, False
    ...
    def generate_plate(self, enhance=False):
        ...
        img_plate_model = cv2.resize(img_plate_model, (530, height))
